...
type ToolbarComponent = DatePickerProps['ToolbarComponent'];
type GetComponentProps<T> = T extends ComponentType<infer P> ? P : never;
type ToolbarComponentProps = GetComponentProps<ToolbarComponent>;

const PickerToolbar: ToolbarComponent = ({ date }: ToolbarComponentProps) => {
  const classes = useStyles();
  const { t } = useWebTranslation(['variables', 'elasticSearch']);
...
const ElasticSearch: VoidFunctionComponent = () => {
  ...
  const handleAddDateOnChange: DatePickerProps['onChange'] = useCallback((newValue) => {
    setAddRecordFrom(newValue === null ? undefined : newValue);
  }, []);
  ...
  const renderDeviceType = useCallback(
    (rowData: RowData) => tDevice(rowData.deviceType),
    [tDevice],
  );

  const renderSensorType = useCallback(
    (rowData: RowData) => tSensorType(rowData.sensorType),
    [tSensorType],
  );
