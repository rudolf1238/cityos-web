...
import type { BaseMenuItemProps, RequireOnlyOne } from './NestedList';
...
const MainLayout: FunctionComponent<{
  ...
  const primaryMenuItems = useMemo(() => {
    ...
    if (process.env.NEXT_PUBLIC_FEAT_AUTOMATION) {
      ...
      automationItems.push(
        {
          nodeId: 'eventLog',
          labelText: t('mainLayout:Event Log'),
          link: '/event-log',
        },
        {
          nodeId: 'notificationSettings',
          labelText: t('mainLayout:Notification Settings'),
      ...
      features.push({
        ...
        labelText: t('mainLayout:Event Trigger'),
  ...
  const secondaryMenuItems = useMemo(() => {
    ...
    if (process.env.NEXT_PUBLIC_FEAT_ABNORMAL) {
      features.push({
        nodeId: 'abnormal',
  ...
  const tertiaryMenuItems = useMemo(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      features.push({
        nodeId: 'sample',
        labelIcon: <PageviewIcon style={{ marginBottom: -1, paddingRight: -1, fontSize: 26 }} />,
    ...
    if (process.env.NEXT_PUBLIC_FEAT_INDOOR && process.env.NEXT_PUBLIC_ENABLE_MOBILE) {
      features.push({
        nodeId: 'indoor-mobile',
        labelIcon: (
          <MobileFriendlyIcon style={{ marginBottom: -1, marginLeft: -1, fontSize: 22 }} />
        ),
        labelText: t('common:Mobile'),
        subItems: [
          {
            nodeId: 'indoor-mobile-list',
            labelText: t('common:List'),
            link: `/mobile/indoor/list`,
          },
          {
            nodeId: 'indoor-mobile-map',
            labelText: t('common:Map'),
        ...
        <AppBar position="fixed" color="inherit" className={classes.appBar} elevation={8}>
          <Toolbar disableGutters className={classes.toolbar}>
            ...
            <div className={classes.rightMenu}>
              ...
              <IconButton disableRipple className={classes.profile} onClick={handleProfileClick}>
                <ProfileAvatar username={profile?.name} photoId={profile?.photo} />
              </IconButton>
            </div>
