...
import { msOfMinute } from '../../libs/constants';
...
import type { IDevice } from '../../libs/schema';
...
/** in milliseconds */
const timingAdvanceKeepAlive = msOfMinute;
/** in seconds */
const streamBuffer = 2;
...
const LiveStreamPlayer: VoidFunctionComponent<VideoPlayerProps> = ({
  ...
  const setRefetchTimer = useTimeout();
  ...
  const onPlay = useCallback<ReactEventHandler<HTMLVideoElement>>(() => {
    ...
    if (!videoNode) return;
    videoNode.currentTime = Math.max(videoNode.currentTime, videoNode.duration - streamBuffer);
  }, []);
