...
const useStyles = makeStyles((theme) => ({
  ...
  content: {
    ...
    // content height: 100vh - (MainToolbar + PageTitle + SplitScreensToolbar)
    // maxWidth: (height - paddingTopBottom - borderTopBottom of SingleScreen) / 9 * 16 + paddingLeftRight + borderLeftRight of SingleScreen
    maxWidth: `calc((var(--vh) * 100 - 230px - ${theme.spacing(
      2,
    )}px - 32px) / 9 * 16 + ${theme.spacing(4)}px + 32px)`,
  },

  playbackContent: {
    // back-calculation for maxWidth by content height & aspectRatio
    // content height: 100vh - (MainToolbar + PageTitle + SplitScreensToolbar + PlaybackVideoToolbar)
    // maxWidth: (height - paddingTopBottom - borderTopBottom of SingleScreen) / 9 * 16 + paddingLeftRight + borderLeftRight of SingleScreen
    maxWidth: `calc((var(--vh) * 100 - 370px - ${theme.spacing(
      2,
    )}px - 32px ) / 9 * 16 + ${theme.spacing(4)}px + 32px)`,
  },
}));
...
const SplitScreens: VoidFunctionComponent<SplitScreensProps> = ({
  ...
  // Pause if all videos can't play and don't have next available clip
  useEffect(() => {
    if (
  ...
  }, [setIsPlaybackPaused, videoStatusRecord]);
