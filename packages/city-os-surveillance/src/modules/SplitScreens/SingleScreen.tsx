...
import React, { CSSProperties, VoidFunctionComponent, memo, useCallback, useState } from 'react';
...
import { downloadDurationLimit } from 'city-os-common/libs/parsedENV';
import { msOfMinute } from 'city-os-common/libs/constants';
import useHiddenStyles from 'city-os-common/styles/hidden';
import useIsEnableRule from 'city-os-common/hooks/useIsEnableRule';
import type { CutRange, IDevice } from 'city-os-common/libs/schema';

import AspectRatio from 'city-os-common/modules/AspectRatio';
import DownloadChecker from 'city-os-common/modules/DownloadChecker';
...
const Player: VoidFunctionComponent<PlayerProps> = memo(
  ({ device, selectedLabel, isLeader, isFixed, onToggleFixed }: PlayerProps) => {
    ...
    const [defaultCutRange, setDefaultCutRange] = useState<CutRange>();
    const [frozenTime, setFrozenTime] = useState<number | null>(null);
    const [openDownloadChecker, setOpenDownloadChecker] = useState(false);
    ...
    const handleToggleScreen = useCallback(
      (isFullPage: boolean) => {
        ...
        if (isFullPage) {
          ...
          setDefaultCutRange(undefined);
        }
      },
      [playbackTime],
    );

    const handleDownloadCheckerOnOpen = useCallback(() => {
      setOpenDownloadChecker(true);
    }, []);

    const handleDownloadCheckerOnClose = useCallback(
      (isEnableDownload: boolean) => {
        setOpenDownloadChecker(false);
        if (!isEnableDownload || !playbackRange) return;
        setDefaultCutRange([
          playbackRange.from.getTime(),
          Math.min(
            playbackRange.to.getTime(),
            playbackRange.from.getTime() + downloadDurationLimit * msOfMinute,
          ),
        ]);
        handleToggleScreen(true);
      },
      [playbackRange, handleToggleScreen],
    );
      ...
      <>
        {showFullPage && frozenTime && (
          <FullPageVideoPlayer
            ...
            defaultCutRange={defaultCutRange}
        ...
        <VideoPlayer
          ...
          onToggleScreen={handleToggleScreen}
          onDownload={handleDownloadCheckerOnOpen}
          header={renderVideoHeader}
        />
        <DownloadChecker open={openDownloadChecker} onClose={handleDownloadCheckerOnClose} />
      </>
    );
  },
);
