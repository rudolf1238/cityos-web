...
import type { LiveViewDevice, PlaybackRange, VideoStatusRecord } from '../libs/type';
import type { SurveillanceContextValue } from './SurveillanceProvider';
...
import SurveillanceProvider from './SurveillanceProvider';
  ...
  useQuery<ReadLiveViewConfigResponse>(READ_LIVE_VIEW_CONFIG, {
    ...
    onCompleted: ({ readLiveViewConfig }) => {
      ...
      if (devices.length > 0) {
        setMode('splitScreen');
        setSelectedDevices(
          getNormalizedDevices({
            devices,
            splitMode: initSplitMode || splitMode,
          }),
        );
      } else {
        setMode('map');
