...
import { DeviceType, SortField, SortOrder } from 'city-os-common/libs/schema';
...
const PoleMenu: VoidFunctionComponent<PoleMenuProps> = ({
  ...
  const onSearch = useCallback(
    (searchWord: string | null) => {
      setKeyword(searchWord ?? undefined);
      clearAllSelected();
    },
  ...
  }, [isSelectAll, clearAllSelected, selectAllPoles, isMountedRef, setIsSelectAll]);

  const handleOnBottomHit = useCallback(() => {
    if (data?.searchDevices.pageInfo.hasNextPage && !loading) void fetchNextPage();
