...
import { DeviceStatus } from 'city-os-common/libs/schema';
import useSubscribeDevicesStatus from 'city-os-common/hooks/useSubscribeDevicesStatus';
import type { GPSPoint, IDevice } from 'city-os-common/libs/schema';
...
const PoleMarker: VoidFunctionComponent<PoleMarkerProps> = ({
  ...
  const svgSize = isSelected ? 80 : 40;

  const svg = useMemo(
    () => `<svg xmlns="http://www.w3.org/2000/svg" width="${svgSize}px" height="${svgSize}px" viewBox="${
      isSelected ? '-20 -20 80 80' : '0 0 40 40'
    }">
    ${
      isSelected
    ...
    ${
      status === 'ERROR'
        ? '<circle fill="#F65860" cx="34.5" cy="7.5" r="5.5" stroke="#FFF" stroke-width="2"/>'
        : ''
    }
  </svg>`,
    [isSelected, status, svgSize, theme.palette.gadget.offline],
  );

  const markerIcon = useMemo(
    () =>
      icon({
        iconAnchor: isSelected ? point(40, 40) : point(20, 20),
