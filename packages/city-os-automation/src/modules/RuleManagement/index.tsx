...
import type { Column } from 'city-os-common/modules/CollapsibleTable';
...
import CollapsibleTable from 'city-os-common/modules/CollapsibleTable';
...
import { RuleSortField } from '../../libs/type';
import { SEARCH_RULES, SearchRulesPayload, SearchRulesResponse } from '../../api/searchRules';
import { ruleNameMaxLength } from '../../libs/constants';
import useAutomationTranslation from '../../hooks/useAutomationTranslation';
import type { EffectiveAt, PartialRuleAutomation, RuleAutomation } from '../../libs/type';
...
const NameCell: VoidFunctionComponent<NameCellProps> = ({ rowData, isCollapse }: NameCellProps) => {
  ...
  const [editRule] = useMutation<EditRuleResponse, EditRulePayload>(EDIT_RULE, {
    ...
    onError: (error) => {
      dispatch({
        ...
        payload: {
          severity: 'error',
          message: isGqlError(error, ErrorCode.RULE_NAME_DUPLICATED)
            ? t(
                'automation:There is already the same rule name in this division_ For differentiating these two, please rewrite it_',
              )
            : t('common:Failed to set value_ Please try again_'),
        },
      });
  ...
  }, [isCollapse, rowData.name, rowData.id, value, editRule]);
  ...
  return (
    <div className={classes.nameCell}>
      <InputBase
        ...
        onBlur={handleUpdate}
        multiline
        inputProps={{ maxLength: ruleNameMaxLength }}
        classes={{
...
const RuleManagement: VoidFunctionComponent = () => {
    ...
    () => [
      ...
      },
      {
        ...
        render: (rowData: RowData, isCollapse: boolean) => (
          <NameCell rowData={rowData} isCollapse={isCollapse} />
        ),
      },
      {
...
export default memo(RuleManagement);
