...
import { SubscriptionSortField } from '../../libs/type';
import useAutomationTranslation from '../../hooks/useAutomationTranslation';
import type { EffectiveAt, PartialRuleSubscription } from '../../libs/type';
...
const NotificationSettings: VoidFunctionComponent = () => {
  const { t } = useAutomationTranslation(['mainLayout', 'automation', 'variables']);
  const classes = useStyles();
  const router = useRouter();
  const changeRoute = useChangeRoute<Query>('/notification-settings');
  ...
  const {
    data: searchSubscriptionsData,
    refetch: refetchSearchSubscriptions,
    loading,
  } = useQuery<SearchMySubscriptionsResponse, SearchMySubscriptionsPayload>(
    SEARCH_MY_SUBSCRIPTIONS,
    {
  ...
  const columns = useMemo<Column<RowData>[]>(
    () => [
      ...
      {
        title: t('automation:Notify by LINE'),
        field: 'byLine',
        textWrap: 'nowrap',
      ...
      {
        title: t('automation:Notify by Email'),
        field: 'byMail',
        textWrap: 'nowrap',
  ...
  return (
    <I18nAutomationProvider>
      <MainLayout>
        <PageContainer>
          <Header title={t('mainLayout:Notification Settings')} />
