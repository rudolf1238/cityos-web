...
import { AuditLogSortField } from '../../libs/type';
import {
  SEARCH_AUDIT_LOGS,
...
import type { RuleAuditLog } from '../../libs/type';
...
const EventLog: VoidFunctionComponent = () => {
  const { t } = useAutomationTranslation(['mainLayout', 'automation', 'variables']);
  const classes = useStyles();
  const router = useRouter();
  const changeRoute = useChangeRoute<Query>('/event-log');
  ...
  const columns = useMemo<Column<RowData>[]>(
    () => [
      ...
      },
      {
        ...
        render: ({ ruleName }: RowData) => <div className={classes.nameCell}>{ruleName}</div>,
      },
      {
  ...
  return (
    <I18nAutomationProvider>
      <MainLayout>
        <PageContainer>
          <Header title={t('mainLayout:Event Log')} />
