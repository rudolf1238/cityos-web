...
import { Building, DetailMode, Floor, Query } from '../../libs/type';
import {
  GET_FULL_BUILDINGS,
...
import DeviceMapMode from '../../modules/detailMode/DeviceMapMode';
import EditBuildingDialog from '../../modules/dialog/EditBuildingDialog';
import FloorSelector from '../../modules/custom/FloorSelector';
import Header from '../../modules/custom/Header';
import HeatMapMode from '../../modules/detailMode/HeatMapMode';
import I18nIndoorProvider from '../../modules/I18nIndoorProvider';
import SplitScreenIcon from '../../assets/icon/split-screen.svg';
import SplitScreenMode from '../../modules/detailMode/SplitScreenMode';
import ThemeButtonMenu, { ThemeButtonMoreMenuItem } from '../../modules/custom/ThemeButtonMenu';
import ThemeIconButtonMoreMenu from '../../modules/custom/ThemeIconButtonMoreMenu';
import ViewerPageProvider, { ViewerPageContextValue } from '../../modules/ViewerPageProvider';

// 因為刪除建築不知道要從哪個 division 刪除，所以先隱藏按鈕，直到這個邏輯問題改善後再開放
// import DeleteBuildingDialog from 'modules/deleteBuilding/DeleteBuildingDialog';
...
const IndoorDetailViewerPage: VoidFunctionComponent = () => {
  ...
  const [selectedFloorNumber, setSelectedFloorNumber] = useState<string | null>(null);
  ...
  const canModifyDevice = useIsEnableRule({ subject: Subject.DEVICE, action: Action.MODIFY });

  // 因為編輯 device 在地圖上的位置需要 edit device 的權限，所以假如不能編輯 device，就乾脆隱藏編輯平面地圖的權限

  const moreMenuContent = useMemo(
    () => [
      canModify && canModifyDevice
        ? {
            id: 'edit-floorplans',
    ...
    [building?.deviceId, canModify, canModifyDevice, router, t],
  );
