...
import { GridLayoutBreakpoint } from '../libs/type';
  ...
  for (let top = originY; spaces[top] && top < originY + height; top += 1) {
    for (let left = originX; left < originX + width; left += 1) {
      if (spaces[top][left] !== 0) {
        ...
        return findSpace(spaces, width, height, nextOriginX, nextOriginY);
      }
    }
  }
...
const getGadgetAddedLayouts = (
  ...
  Object.entries(newLayouts).forEach(([sizeKey, currLayouts]) => {
    ...
    const acceptablePosition = findSpace(spaces, width, height, 0, 0);
  ...
  Object.keys(newLayouts).forEach((sizeKey) => {
    ...
    if (breakpoint) {
      ...
      if (targetIndex >= 0) {
        ...
        const acceptablePosition = findSpace(spaces, width, height, 0, currLayouts[targetIndex].y);
        const newPosition = isLayoutPosition(acceptablePosition)
          ? acceptablePosition
          : { x: 0, y: spaces.length };
...
const Dashboard: VoidFunctionComponent = () => {
  ...
  const enableAdd = allGadgetsConfig[tabIdx].length < gadgetLimit;
  ...
  const { loading, called } = useQuery<ReadDashboardResponse>(READ_DASHBOARD, {
    ...
    onCompleted: ({ readDashboard }) => {
      ...
      for (let i = 0; i < tabAmount; i += 1) {
        const targetConfig = readDashboard.find(({ index }) => index === i)?.config;
        const parsedConfig = targetConfig ? parseConfigToJson(targetConfig) : null;
        setConfig(parsedConfig || emptyConfig, i);
      }
    },
  });
  ...
  const handleUpdateConfig = useCallback(
    (newGadgetConfig: GadgetConfig<ConfigFormType>) => {
      const {
        id: selectedId,
        width: newWidth,
        height: newHeight,
        ...
        <Guard subject={Subject.DASHBOARD} action={Action.VIEW}>
          <Container ref={setContainerNode} tabIndex={0} maxWidth={false} className={classes.root}>
            <div className={classes.bar}>
              <MiniTabs number={tabAmount} index={tabIdx} onChange={handleTabsChange} />
              <ThemeIconButton
                color="primary"
                size="small"
