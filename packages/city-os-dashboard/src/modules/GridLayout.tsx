...
import {
  ...
  isDevicesTitle,
  isDevicesTitleLayout,
  isDivisionLayout,
  isDivisionOnly,
} from '../libs/validators';
import useDashboardTranslation from '../hooks/useDashboardTranslation';
import type { GadgetBaseProps } from './Gadgets/GadgetBase';

import AqiMonitor from './Gadgets/AqiMonitor';
import CarFlow from './Gadgets/CarFlow';
import CarFlowPredict from './Gadgets/CarFlowPredict';
...
import EnergyConsumption from './Gadgets/EnergyConsumption';
...
import TodayEnergyConsumption from './Gadgets/TodayEnergyConsumption';
...
const GadgetContent: VoidFunctionComponent<GadgetContentProps> = memo(
  ({ config, gadgetProps }: GadgetContentProps) => {
    switch (config.type) {
      ...
      case GadgetType.CAR_FLOW_PREDICT:
        return isDeviceDurationLayout(config) ? (
          <CarFlowPredict {...gadgetProps} config={config} />
        ) : null;
      ...
      case GadgetType.TODAY_ENERGY_CONSUMPTION:
        return isDevicesTitleLayout(config) ? (
          <TodayEnergyConsumption {...gadgetProps} config={config} />
        ) : null;
      case GadgetType.ENERGY_CONSUMPTION:
        return isDevicesTitle(config) ? (
          <EnergyConsumption {...gadgetProps} config={config} />
        ) : null;
      default:
        return null;
    }
