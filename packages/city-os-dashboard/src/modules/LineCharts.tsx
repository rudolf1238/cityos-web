...
const getTooltipData = ({
  ...
  curves.map(({ key, label, color, points, isSecondCurve }) => {
    const closestPoint = getClosestPoint({ points, currentTime });
    return {
      key: `${key}${closestPoint?.time.toString() || ''}`,
      label,
      color,
      point: closestPoint,
      isSecondCurve,
      isPastCurve: key.indexOf('past') > -1,
    };
  });
...
const LineCharts: VoidFunctionComponent<LineChartsProps> = ({
  ...
  const tooltipTimeConvert = (time: number, isSecondCurve?: boolean, isPastCurve?: boolean) => {
    if (isSecondCurve) {
      if (isPastCurve) {
        return time - durationConfig.timeScaleRange;
      }
      return time + durationConfig.timeOfDuration;
    }
    return time;
  };
  ...
  const maxY = useMemo(() => {
    ...
    if (secondCurve) {
      secondCurve.points.forEach(({ value }) => {
        if (value > maxValue) {
          maxValue = value;
        }
      });
  ...
  }, [filteredCurves, secondCurve]);

  // in order to put second curve on same axis, recalculate second curve point to current time
  const normalizeSecondCurve = useMemo(() => {
    if (!secondCurve || secondCurve.points.length === 0) return null;
    const normalizePoints =
      secondCurve.key.indexOf('past') > -1
        ? secondCurve?.points
            .map((point) => ({ ...point, time: point.time + durationConfig.timeScaleRange }))
            .filter(
              (point) =>
                point.time >= startTime && point.time <= startTime + durationConfig.timeScaleRange,
            )
        : secondCurve?.points
            .map((point) => ({ ...point, time: point.time - durationConfig.timeOfDuration }))
            .filter(
              (point) =>
                point.time >= startTime && point.time <= startTime + durationConfig.timeScaleRange,
            );
  ...
  }, [durationConfig.timeScaleRange, durationConfig.timeOfDuration, secondCurve, startTime]);
    ...
    <ParentSize>
      {({ width, height }) => {
        ...
        const handleTooltip = (event: TouchEvent<SVGRectElement> | MouseEvent<SVGRectElement>) => {
          ...
          const combinedCurves = normalizeSecondCurve
            ? [{ ...normalizeSecondCurve, isSecondCurve: true }, ...filteredCurves]
            : filteredCurves;
          ...
          <div className={classes.root}>
            <svg width={width} height={height} className={classes.svg}>
              <Group left={margin.left} top={margin.top}>
                ...
                />
                {normalizeSecondCurve && (
                  <LineChart
                    variant="pastCurve"
                    timeScale={timeScale}
                    valueScale={valueScale}
                    points={normalizeSecondCurve.points}
                  />
                )}
                {normalizeSecondCurve?.points.length === 1 && (
                  <circle
                    cx={timeScale(normalizeSecondCurve.points[0].time)}
                    cy={valueScale(normalizeSecondCurve.points[0].value)}
                    r={4}
                    fill="white"
                ...
                {tooltipData?.map(({ key, point, color, isSecondCurve }, idx) =>
                  point ? (
                    <circle
                      key={key}
            ...
            {tooltipData && (
              ...
              >
                {tooltipData.map(
                  ({ key, label, color, point, isSecondCurve, isPastCurve }, idx) => (
                    <div key={key} className={classes.tooltipItem}>
                      <div
                        className={classes.squareIcon}
                        style={{
                          backgroundColor: isSecondCurve
                            ? theme.palette.info.main
                            : theme.palette.gadget[color || defaultColors[idx]],
                        }}
                      />
                      {labelType !== 'none' && (
                        <Typography variant="overline">
                          {labelType === 'time' && point
                            ? timeTickParser(
                                tooltipTimeConvert(point.time, isSecondCurve, isPastCurve),
                              )
                            : label}
                        </Typography>
                      )}
