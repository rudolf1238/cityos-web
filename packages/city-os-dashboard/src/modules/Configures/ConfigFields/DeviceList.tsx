...
interface DeviceListProps {
  ...
  bulletColorList?: string[];
  circleBullet?: boolean;
}
...
const DeviceList: VoidFunctionComponent<DeviceListProps> = ({
  ...
  return (
    <div aria-label={t('column:Device Name')} className={classes.list}>
      ...
      >
        {deviceLimit
          ? `${t('column:Device Name')} (${devices.length} OF ${deviceLimit})`
      ...
      {devices.map(({ label, value: deviceId }, index) => (
        <ListItem key={deviceId} className={classes.listItem}>
          {!disableBullet && (
            <div
              className={clsx(classes.bullet, { [classes.circleBullet]: circleBullet })}
              style={{
                backgroundColor:
                  bulletColorList?.[index] || theme.palette.gadget[defaultColors[index]],
              }}
            />
          )}
          <ListItemText
