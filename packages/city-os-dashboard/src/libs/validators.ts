...
const DevicesTitleLayoutJsonSchema: JSONSchemaType<
  GadgetConfig<ConfigFormType.DEVICES_TITLE_LAYOUT>
> = {
  type: 'object',
  additionalProperties: false,
  required: ['id', 'width', 'height', 'type', 'setting'],
  properties: {
    ...
    width: { type: 'number', minimum: 1, maximum: 2 },
    height: { type: 'number', minimum: 1, maximum: 2 },
    type: { type: 'string', enum: configFormSet[ConfigFormType.DEVICES_TITLE_LAYOUT] },
    setting: {
      type: 'object',
      additionalProperties: false,
      required: ['title', 'deviceIds', 'size'],
      properties: {
        title: { type: 'string' },
        deviceIds: { type: 'array', items: { type: 'string' } },
        size: { type: 'string', enum: Object.values(GadgetSize) },
...
export const isDevicesTitleLayout = ajv.compile(DevicesTitleLayoutJsonSchema);
