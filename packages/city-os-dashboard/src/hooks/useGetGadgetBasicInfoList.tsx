...
const useGetGadgetInfoList = (): BasicGadgetInfo[] => {
  ...
  const originGadgets = useMemo<BasicGadgetInfo[]>(
    () => [
      ...
      {
        name: tGadget(GadgetType.CAR_FLOW_PREDICT),
        type: GadgetType.CAR_FLOW_PREDICT,
        deviceType: DeviceType.CAMERA,
        attribute: {
          key: 'recognition_type',
          value: RecognitionKey.CAR_FLOW,
        ...
        description: t(
          'Display car flow results from a specific period of time covering a specific camera_ The solid line is values from the last period; the dotted line is the prediction for the period after that_',
        ),
        size: GadgetSize.SQUARE,
      },
      {
      ...
      {
        name: tGadget(GadgetType.TODAY_ENERGY_CONSUMPTION),
        type: GadgetType.TODAY_ENERGY_CONSUMPTION,
        deviceType: DeviceType.BANPU_INDOOR_METER,
        description: t(
          'Show the hourly power consumption of 3 devices (24 hours)_ and can customize the name (e_g_ Solar)_',
        ),
        size: GadgetSize.SQUARE,
      },
      {
        name: tGadget(GadgetType.ENERGY_CONSUMPTION),
        type: GadgetType.ENERGY_CONSUMPTION,
        deviceType: DeviceType.BANPU_INDOOR_METER,
        description: t(
          'Displays the percentage of indoor power consumption_ up to 6 devices can be selected_ presented in a pie chart_ the sum is 100%_ and when the mouse hovers on the pie chart it will display the actual power kW_',
        ),
        size: GadgetSize.DEFAULT,
      },
    ],
